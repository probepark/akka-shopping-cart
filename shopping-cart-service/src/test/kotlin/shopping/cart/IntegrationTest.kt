...
akka.discovery.config.services.shopping-cart-service.endpoints = [
  ...
  { host = "127.0.0.1", port = ${managementPorts[2]}},
        ...
        fun setup() {
            ...
            init(testNode1!!.testKit.system())
            init(testNode2!!.testKit.system())
            init(testNode3!!.testKit.system())
                        ...
                        upProbe.awaitAssert(
                                ...
                                Creator<Any?> {
                                    ...
                                    cluster
                                            ...
                                            .members
                                            .iterator()
                                            .forEachRemaining { member: Member -> Assert.assertEquals(MemberStatus.up(), member.status()) }
                                    null
                                })
                    })
        }
    ...
    private class TestNodeFixture(grpcPort: Int, managementPorts: List<Int>, managementPortIndex: Int) {
        val testKit: ActorTestKit =
                ActorTestKit.create("IntegrationTest",
                        nodeConfig(grpcPort, managementPorts, managementPortIndex)
                                .withFallback(sharedConfig()))

        val system: ActorSystem<*> = testKit.system()
        private val clientSettings: GrpcClientSettings = GrpcClientSettings.connectToServiceAt("127.0.0.1", grpcPort, system).withTls(false)
